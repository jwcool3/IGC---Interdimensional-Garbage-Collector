...
public class CombatUI : MonoBehaviour
{
    ...
    // Track the discovered ship we're fighting
    ...
    private void Start()
    {
        ...
        // Subscribe to ship interaction events for discovered ship tracking
        SubscribeToShipInteractionEvents();
    ...
    private void OnDestroy()
    {
        ...
        UnsubscribeFromShipInteractionEvents();
    }

    /// <summary>
    /// Subscribe to ship interaction events to track discovered ships
    /// </summary>
    private void SubscribeToShipInteractionEvents()
    {
        if (ShipInteractionManager.Instance != null)
        {
            // We'll add a custom event to ShipInteractionManager for this
            Debug.Log("CombatUI: Subscribed to ship interaction events");
        }
    }

    /// <summary>
    /// Unsubscribe from ship interaction events
    /// </summary>
    private void UnsubscribeFromShipInteractionEvents()
    {
        if (ShipInteractionManager.Instance != null)
        {
            Debug.Log("CombatUI: Unsubscribed from ship interaction events");
        }
    }

    /// <summary>
    /// Set the discovered ship that we're currently fighting
    /// Called by ShipInteractionManager when starting combat with a discovered ship
    /// </summary>
    public void SetCurrentDiscoveredShip(DiscoveredShip ship)
    {
        currentDiscoveredShip = ship;
        Debug.Log($"CombatUI: Set current discovered ship to {ship?.ShipName ?? "null"}");
    ...
    /// <summary>
    /// Clear the current discovered ship reference
    /// </summary>
    public void ClearCurrentDiscoveredShip()
    {
        currentDiscoveredShip = null;
        Debug.Log("CombatUI: Cleared current discovered ship");
    }
    ...
    public void UpdateEnemyDisplay()
    {
        ...
        // Enhanced enemy ship image handling
        UpdateEnemyShipImage(enemy);
    }

    /// <summary>
    /// Enhanced enemy ship image update with discovered ship integration
    /// </summary>
    private void UpdateEnemyShipImage(EnemyShip enemy)
    {
        if (enemyShipImage == null) return;

        Sprite shipIcon = null;
        bool useDiscoveredShipImage = false;

        // Priority 1: If this is a discovered ship combat, use the discovered ship's image
        if (CombatManager.Instance.isSingleShipMode && currentDiscoveredShip != null)
        {
            Debug.Log($"CombatUI: Using discovered ship image for {currentDiscoveredShip.ShipName}");
            
            if (currentDiscoveredShip.ShipIcon != null)
            {
                shipIcon = currentDiscoveredShip.ShipIcon;
                useDiscoveredShipImage = true;
                Debug.Log("CombatUI: Using discovered ship's assigned icon");
            }
            else
            {
                // Try to get image based on discovered ship's properties
                shipIcon = GetDiscoveredShipImage(currentDiscoveredShip);
                if (shipIcon != null)
                {
                    useDiscoveredShipImage = true;
                    Debug.Log("CombatUI: Using discovered ship's generated icon");
                }
            }
        }

        // Priority 2: Try getting ship icon directly by model name if available
        if (shipIcon == null && !string.IsNullOrEmpty(enemy.shipModelName))
        {
            if (EnemyIconManager.Instance != null)
            {
                ...
                if (shipIcon != null)
                {
                    Debug.Log($"CombatUI: Using ship model icon for {enemy.shipModelName}");
                }
            }
        }

        // Priority 3: If no icon found by model name, try getting by type and sector
        if (shipIcon == null && EnemyIconManager.Instance != null)
        {
            ...
            if (shipIcon != null)
            {
                Debug.Log("CombatUI: Using EnemyIconManager icon");
            }
        }
        ...
        if (shipIcon != null)
        {
            ...
            // Use discovered ship color if applicable, otherwise white
            if (useDiscoveredShipImage && currentDiscoveredShip != null)
            {
                enemyShipImage.color = currentDiscoveredShip.ShipColor;
                Debug.Log($"CombatUI: Applied discovered ship color: {currentDiscoveredShip.ShipColor}");
            }
            else
            {
        ...
        {
            // Fallback: Use color-coded display
            enemyShipImage.enabled = true;
            enemyShipImage.sprite = null; // Use solid color

            Color fallbackColor;
            
            // Use discovered ship color if available
            if (currentDiscoveredShip != null)
            {
                fallbackColor = currentDiscoveredShip.GetRarityColor();
                Debug.Log($"CombatUI: Using discovered ship rarity color: {fallbackColor}");
            }
            else if (EnemyIconManager.Instance != null)
            {
                fallbackColor = EnemyIconManager.Instance.GetColorForEnemyType(enemy.type);
            }
            else
            {
                ...
                switch (enemy.type)
                {
                    case EnemyType.Scavenger:
                        fallbackColor = new Color(0.5f, 0.5f, 0.5f); // Gray
                        break;
                    case EnemyType.Rival:
                        fallbackColor = new Color(0.2f, 0.6f, 1f); // Blue
                        break;
                    case EnemyType.Anomaly:
                        fallbackColor = new Color(1f, 0.4f, 0.8f); // Pink
                        break;
                    case EnemyType.Boss:
                        fallbackColor = new Color(1f, 0.2f, 0.2f); // Red
                        break;
                    default:
            ...
            enemyShipImage.color = fallbackColor;
        }
    }

    /// <summary>
    /// Get image for discovered ship using similar logic to scanner UI
    /// </summary>
    private Sprite GetDiscoveredShipImage(DiscoveredShip ship)
    {
        // Try to load from Resources based on ship name
        if (!string.IsNullOrEmpty(ship.ShipName))
        {
            ...
            {
                Debug.Log($"CombatUI: Found ship icon in Resources for {cleanName}");
                return resourceIcon;
            }
        }

        // Try to get icon from EnemyIconManager based on ship type
        if (EnemyIconManager.Instance != null)
        {
            ...
            if (typeIcon != null)
            {
                Debug.Log($"CombatUI: Found type icon for {enemyType}");
                return typeIcon;
            }
            ...
            if (!string.IsNullOrEmpty(ship.ShipType))
            {
                ...
                if (modelIcon != null)
                {
                    Debug.Log($"CombatUI: Found model icon for {ship.ShipType}");
                    return modelIcon;
                }
            }
        }
    ...
    public void UpdateZoneDisplay()
    {
        ...
        if (zoneNameText != null)
        {
            ...
            {
                // Show discovered ship name if available
                string encounterName = currentDiscoveredShip?.ShipName ?? "Unknown Ship";
                zoneNameText.text = $"[ENCOUNTER] {encounterName}";
        ...
        if (zoneProgressText != null)
        {
            if (CombatManager.Instance.isSingleShipMode)
            {
                string shipInfo = currentDiscoveredShip != null ? 
                    $"{currentDiscoveredShip.GetRarityDisplayText()} - Level {currentDiscoveredShip.Level}" : 
                    "Single Ship Encounter";
                zoneProgressText.text = shipInfo;
                zoneProgressText.color = currentDiscoveredShip?.GetRarityColor() ?? Color.yellow;
            }
            else
            {
    ...
    {
        // Clear discovered ship reference when retreating
        ClearCurrentDiscoveredShip();
    ...
    private void OnZoneSelected(int index)
    {
        ...
        {
            // Clear discovered ship reference when changing zones
            ClearCurrentDiscoveredShip();
